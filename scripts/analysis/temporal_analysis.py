# Script to generate light curves for time variability analysis of H.E.S.S. data
# NOTE: To obtain Fermi-LAT light curves, it is recommended to run the `gta.lightcurve` routine in FermiPy
# As an alternative, Fermi-LAT lightcurves are publicly available at: https://fermi.gsfc.nasa.gov/ssc/data/access/lat/LightCurveRepository/
...
from    alpsup.paths                import  gen_dirs, get_results_dir, CONFIGS_DIR
...
def get_ethresh(datasets: Datasets):
    """
    Compute threshold energy for all datasets, including minimum, maximum, and average threshold energies.
    Args:
...
def time_resolved_spectroscopy(datasets: Datasets, table: Table, use_edec: bool = True, check_convergence: bool = False) -> Table:
    """
    ...
    for k, time in enumerate(time_intervals):
        ...
        # NOTE: Display fit parameters
        print(f"Index: {dataset_to_fit.models[target].parameters["index"].value} +/- {dataset_to_fit.models[target].parameters["index"].error}")
        print(f"Normalization: {dataset_to_fit.models[target].parameters["amplitude"].value} +/- {dataset_to_fit.models[target].parameters["amplitude"].error}")
        print(f"Reference: {dataset_to_fit.models[target].parameters["reference"].value}")
...
if __name__ == "__main__":
    ...
    dataset_obs, dataset = get_hess_dataset(target, dataset = args.dataset, config = args.config,
                                            time_select_min = kwargs.get("time_min", None), time_select_max = kwargs.get("time_max", None))
    ...
    if edec != edec_gp:
        log.warning("Decorrelation energy mismatch between methods, results may be unreliable!")

    # Set new reference energy
    dataset.models[target].parameters["reference"].quantity = edec

    # Run the fit again if reference has been changed (don't re-compute if specific reference given!)
    ...
    bblocks_edges_flux[0] = sorted(dataset_obs.gti.time_start)[0].mjd
    # Extend end of last block to include full time interval
    bblocks_edges_flux[-1] = sorted(dataset_obs.gti.time_start)[-1].mjd

    # NOTE: Split blocks separated by very long periods with no data (as variability could occur)
    # a.k.a. "gaps in the data" (This affects mainly PKS1510-089, a variable source in Fermi-LAT with variability not visible in HESS data)
    time_starts = sorted(dataset_obs.gti.time_start)
    for i in range(len(time_starts) - 1):
        if (time_starts[i + 1].mjd - time_starts[i].mjd) > 2500:
            log.warning("Found long gap in data!")
            # NOTE: For now, split it along the middle! This might be a problem for Fermi-LAT analysis later on!
            time_mid = (time_starts[i + 1].mjd + time_starts[i].mjd) / 2.0
            # TODO: This might break if not first block!
            bblocks_edges_flux = np.insert(bblocks_edges_flux, 1, time_mid)
    ...
    # TODO: Plotting uses average flux, but I'm still re-computing full light-curve here!
    ...
    lc_flux_tab, _ = time_resolved_spectroscopy(dataset_obs, lc_flux_tab, use_edec = args.use_edec)
    ...
    lc_flux_bblock_tab, lc_flux_bblock_results = time_resolved_spectroscopy(dataset_obs, lc_flux_bblock_tab, check_convergence = True, use_edec = args.use_edec)
    ...
    lc_index_bblock_tab, lc_index_bblock_results = time_resolved_spectroscopy(dataset_obs, lc_index_bblock_tab, check_convergence = True, use_edec = args.use_edec)
    ...
    # ==================================== #
    # COMPUTE ELLIPSES AND BAYESIAN BLOCKS #
    # ==================================== #

    # Extract Bayesian block edges for flux
    bblocks_edges_flux = [lc_flux_bblock_tab["time_min"][0]]
    bblocks_edges_flux.extend(lc_flux_bblock_tab["time_max"])
    ...
    cov_bblock = np.load(file = get_results_dir(target, args.bblock, output = "gamma-out") / "lc_flux_bblock_cov.npy")

    # Generate list containing Bayesian block index for each flux point
    fp_block = np.digitize(lc_flux_bblock_tab["time_ref"].value, bblocks_edges_flux)
    ...
    # Keep track of points, ellipses, and block times
    points, errors, ellipses, ellipses_all, times = [], [], [], [], []

    # For each Bayesian block, compute and store ellipses
    for k, result in enumerate(lc_flux_bblock_tab):
        ...
        # At which sigma level to consider overlapping ellipses
        sigma_overlap = args.sigma_overlap
        ...
        for ic, l in enumerate( [0.68, 0.95, 0.99] ):
            ...
            # Given required signifiance level for time segmentation, keep track of ellipses
            if (sigma_overlap == (ic + 1)):

                # Keep track of the points
                points.append([ index_val, amplitude_val ])
                # Keep track of uncertainties
                errors.append([ index_err, amplitude_err ])
                # Keep track of the times
                times.append([ fit_times_bb_flux[k][0].mjd, fit_times_bb_flux[k][1].mjd ])
                # Keep track of the ellipse axes and rotation
                ellipses.append( [a, b, alpha] )
            # Keep track of the ellipse axes and rotation (for all confidence levels!)
            block_ellipses.append( [a, b, alpha] )
        ...
        ellipses_all.append(block_ellipses)

    # Generate final time segmentation based on overlapping, contiguous Bayesian blocks

    # Helper function for generating ellipse objects
    ...
    # Define shapely ellipse object
    ells = []
    # Go over all ellipses
    for k in range(len(ellipses)):
        # Create shapely ellipses objects for each
        # TODO: FIX INDEX FOR SIGMA VALUE!!!
        ells.append(
            create_ellipse(
                center = ( points[k][1], points[k][0]), 
                axes = ( ellipses[k][1] / 2, ellipses[k][0] / 2 ), 
                inclination = ellipses[k][2], ) )
        
    ellipse_data = {
    "points": np.array(points),
    "errors": np.array(errors),
    "ellipses": np.array(ellipses_all),
    "times": np.array(times), }

    np.save(
        get_results_dir(target, args.bblock, output = "gamma-out") / "lc_ellipses.npy",
        ellipse_data, )

    # =============================== #
    # COMPUTE FINAL TIME SEGMENTATION #
    # =============================== #

    # Final time segmentation array
    times_final = []

    # Keep track of which ellipses overlap
    inters_empty = []
    # Compute intersection between contiguous ellipses
    for k in range(1, len(ellipses)):
        # Check current versus previous
        inter = intersection(ells[k], ells[k-1])
        # TODO: Plot intersection
        # plot_polygon(inter, ax = ax, add_points=False, fc='red')
        # Is intersection empty
        inters_empty.append(inter.is_empty)

    # NOTE: ALTERNATIVE APPROACH without combining overlapping Bayesian blocks sequentially
    '''
    # Final time segmentation: merge contiguous blocks with overlapping ellipses
    final_times = [list(times[0])]
    for k, empty in enumerate(inters_empty, start=1):
        if not empty:
            final_times[-1][1] = times[k][1]   # overlap — extend current group
        else:
            final_times.append(list(times[k]))  # no overlap — start new group

    bblock_table = Table(rows=final_times, names=["time_min", "time_max"])
    '''

    # NOTE: Combining overlapping Bayesian blocks sequentially
    final_times = [list(times[0])]
    group = [0]
    for k in range(1, len(ells)):
        overlaps_group = any(not intersection(ells[k], ells[j]).is_empty for j in group)
        if overlaps_group:
            final_times[-1][1] = times[k][1]
            group.append(k)
        else:
            final_times.append(list(times[k]))
            group = [k]

    # Define table of final time segmentation
    bblock_table = Table(rows = final_times, names = ["time_min", "time_max"])

        # TODO: SIMPLIFY PLOTTING (RIGHT NOW, RE-COMPUTES ELLIPSES?)

    # NOTE: If only one Bayesian block, extend times to span full GTI interval
    # This is not reflected on the Bayesian blocks on the plot (as these are the results of the computed blocks), only in the final time segmentation values
    if len(bblock_table) <= 1:
        bblock_table = Table(rows = [[sorted(dataset_obs.gti.time_start)[0].mjd, sorted(dataset_obs.gti.time_start)[-1].mjd]], names = ["time_min", "time_max"])

    # Display final time segmentation intervals
    log.info(f"Final Bayesian block time segmentation:\nNumber of blocks: {len(bblock_table)}\n{bblock_table}")

    # ==================================================== #
    # SAVE INFO ON FINAL TIME SEGMENTATION FOR EACH SOURCE #
    # ==================================================== #

    log.info(f"Saving final time segmentation into hess_config.yaml file")
    # Save final time segmentation into sources blocks file
    with open(CONFIGS_DIR / "hess_config.yaml", "r") as f:
        # Load file
        data = yaml.full_load(f)
    # If data file is empty, create initial dictionary
    if data is None:
        data = {}
    # Update entry
    data[target] = {}
    # Save config files
    data[target]["hap_dataset"] = args.dataset
    ...
    plot_lightcurve(target, args.bblock)
    plot_ellipses(target, args.bblock, print_labels = args.annotate, loc = kwargs.get("loc", "default"))
